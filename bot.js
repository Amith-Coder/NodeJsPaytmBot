...
var bot_token = '6235858930:AAGe0M02KS71JouTEQ2551h3PcMrTzMA_mg'; //YOUR BOT TOKEN HERE
...
mongo.connect('mongodb+srv://amithsunil2015:TFUPlecGIQ4R2Sr2@cluster1.psjdysk.mongodb.net/?retryWrites=true&w=majority', { useUnifiedTopology: true }, (err, client) => {
    if (err) {
        console.log(err);
    }
    db = client.db('Demot1');
...
bot.hears(/^\/start (.+[1-9]$)/, async (ctx) => {
    try {
        ...
        if (admin.length == 0) {
            db.collection('admindb').insertOne({ admin: "admin", ref: 1, cur: 'INR', paychannel: '@Notset', bonus: 0.1, minimum: 1, botstat: 'Active', withstat: 'ON', subwallet: 'NOT SET', MKEY: 'NOT SET', MID: 'NOT SET', channels: [] })
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users\n\n🔎 Coded By: <a href="tg://user?id=2131044278">Coder🔥</a></b>' , { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                } else {
